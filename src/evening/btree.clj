...
(defn leaf-node [name data dirty? width]
  (LeafNode. name data dirty? width))
...
(defn branch-node [name children dirty? width]
  (BranchNode. name children dirty? width))
...
;(def fubar-branch (BranchNode. "fubar-branch" [foo-leaf bar-leaf] true 40))
